...
describe('AMI limits', () => {
  ...
  it('getIncomeLimit returns null for unknown pctAmi value (JS/cast callers)', () => {
    // TypeScript union prevents this at compile time but JS callers or casts can bypass it.
    // The default branch ensures number | null is always returned, never undefined.
    expect(getIncomeLimit(4, 40 as 30, 2026)).toBeNull();
  });

  it('getIncomeLimit returns null when no data exists for year', () => {
    expect(getIncomeLimit(4, 80, 2023)).toBeNull();
  });
});
